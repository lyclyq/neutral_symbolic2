...
#     llm_int8_skip_modules=["lm_head"],  # 避免对分类头量化
...
output_dir = "./output/Qwen2-VL-2B-LatexOCR_weighted_2_350_ep3_large_samples"
MAX_LENGTH = 8192
...
def process_func(example):
    ...
    """
    适配 HuggingFace chest_xray14 数据集的处理逻辑
    """
    
    image = example["image"]  # PIL.Image object
    label_indices = example["labels"]  # List[int]
    # print(label_indices)
    # for idx in label_indices:
    #     if idx >= 15:
    #         print(f"⚠️ 出现异常标签：{idx}")
    #         raise ValueError(f"非法标签 {idx}, 超过最大值 14")
    # # 将 label index 转为疾病名

    disease_names = [disease_list[i] for i in label_indices]
    
    output_content = "This chest X-ray image shows: " + ", ".join(disease_names) + "."
    # print(output_content)
    # 构建 prompt
    messages = [{
        "role": "user",
        "content": [
            {"type": "image", "image": image, "resized_height": 350, "resized_width": 350},
    ...
    response_ids = response["input_ids"][0].tolist()
    response_mask = response["attention_mask"][0].tolist()

    input_ids = instruction["input_ids"][0] + response_ids + [tokenizer.pad_token_id]
    attention_mask = instruction["attention_mask"][0] + response_mask + [1]
    labels = [-100] * len(instruction["input_ids"][0]) + response_ids + [tokenizer.pad_token_id]
...
features = Features({
    "image": dataset["train"].features["image"],  # 保留原来的 image 类型
    "labels": Sequence(Value("int32"))  # 🚨 关键：改成纯 int32 序列
})

train_ds = train_ds.cast(features)  # 重新指定字段类型
...
args = TrainingArguments(
    ...
    num_train_epochs=5,                 # 改为 5 epoch
    learning_rate=5e-5,                 # 可选，稍微小一点更稳
    logging_steps=10,                    # 每 step 打印 loss，便于观察 
