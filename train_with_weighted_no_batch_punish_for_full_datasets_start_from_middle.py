...
output_dir ="output/Qwen2-VL-7B-Instruct-output_8epoch_demo_weighted_punish_log_32rank_no_batch"
...
disease_list = [
    ...
    "nodule", "pleural thickening", "pneumonia", "pneumothorax", "default_token"
]
...
count_dict = {
    ...
    "pneumonia":1431,
    "pneumothorax":5302,
    "default_token":112120
}


max_freq = max(count_dict.values())

token_weight_dict = { disease: np.log(1 + (max_freq / count_dict[disease])) for disease in disease_list }

print(token_weight_dict)

default_weight = token_weight_dict["default_token"]
for k in token_weight_dict:
    token_weight_dict[k] /= default_weight
            ...
            for disease in disease_list:
                ...
                for j in range(len(label_seq_list) - len(token_ids) + 1):
                    if label_seq_list[j:j+len(token_ids)] == token_ids:
                        ...
                        token_weights[i, j:j+len(token_ids)] = token_weight_dict[disease]
                        # print(f"✅ Found disease '{disease}' at position {j} with weight {token_weight_dict[disease]}")
...
# train_ds = train_ds.select(range(50))  # 只取前5个样本
...
checkpoint_path = os.path.join('output/Qwen2-VL-7B-Instruct-output_5epoch_demo_weighted_punish_log_32rank_no_batch', "checkpoint-1405")  # 改成你实际的
